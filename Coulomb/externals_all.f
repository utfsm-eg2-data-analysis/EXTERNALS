...
c ------- Write out sbsv(npts)[added] and sigborn_cc [add] and f1cc [add] before ccor ---------   
c----------------------    ccor = sbsv(npts)/sigborn_cc/f1cc/f1cc ---
c          write(17,'(1x,5f9.4,10e15.5)') e0sv(npts),epsv(npts),
c     >          thsv(npts),xsv(npts),q2set,sbsv(npts),sbisv(npts),
c     >          sbqesv(npts),srsv(npts),
c     >          srelsv(npts),srqesv(npts),srinsv(npts),  
c     >          sbsv(npts), ccor
...
C Now open output files
     ...
     > '      Sig_Rad_EL   Sig_Rad_QE   Sig_Rad_DIS  Sig_BornPlsQE  C_cor'
