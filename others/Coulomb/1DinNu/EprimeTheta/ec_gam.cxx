...
void ec_gam::BookHists(void){
 ...
 for(int  nv=0; nv<nvbins;nv++){
  h_etheta_nu[nv]= new TH2F(Form("h_etheta_nu%d",nv),Form("h_etheta_nu%d",nv), 200,0.5,3.,200,15,55.);
 }
...
void ec_gam::FillTables(void){
 ...
 if(qbin>-1&&qbin<nqbins &&vbin>-1&&vbin<nvbins){
  ...
  h_etheta_nu[vbin]->Fill(Eprime, Theta);
 }
...
///////////////////////// NU ///////////////////////
 gStyle->SetOptTitle(11111);
 TCanvas *c2 = new TCanvas("c2","c2",400*3,300);
 c2->Divide(3,1, 0.0001,0.0001);
 for(int  nv=0; nv<nvbins;nv++){
  c2->cd(nv+1);
  h_etheta_nu[nv]->Draw("colz");
  h_etheta_nu[nv]->GetXaxis()->SetTitle("E'(GeV)");
  h_etheta_nu[nv]->GetYaxis()->SetTitle("#theta(deg)"); 
  mean_v_theta[nv]=h_etheta_nu[nv]->GetMean(2);
  mean_v_Eprime[nv]=h_etheta_nu[nv]->GetMean(1);

  cout << nv << " " << " <theta>="<<mean_v_theta[nv] << "  <Eprime>=" <<mean_v_Eprime[nv]<<endl; 
 }
 c2->Print("~/secure/ExternalsRC/ThetaEprime_inNu.png");
 c2->Clear();
 }
