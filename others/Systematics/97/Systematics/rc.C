...
Float_t ratioRC97_CD[nqbins][nvbins], ratioRC97_FeD[nqbins][nvbins], ratioRC97_PbD[nqbins][nvbins];
...
///////////////////////// read qv=97 binning //////////////////
 in.open("eRC_qv97_C.txt");
  for(int nq=0; nq<nqbins;nq++){
 for(int nv=0;  nv<nvbins; nv++){  
   in>>nq>>nv>>rc>>ratioRC97_CD[nq][nv]; 
   }
  }
 in.close();

 cout << " **************** Fe ********************** " << endl; 
 in.open("eRC_qv97_Fe.txt");
  for(int nq=0; nq<nqbins;nq++){
 for(int nv=0;  nv<nvbins; nv++){  
   in>>nq>>nv>>rc>>ratioRC97_FeD[nq][nv]; 
   }
  }
 in.close(); 

 cout << " **************** Pb ********************** " << endl; 
 in.open("eRC_qv97_Pb.txt");
  for(int nq=0; nq<nqbins;nq++){
 for(int nv=0;  nv<nvbins; nv++){  
   in>>nq>>nv>>rc>>ratioRC97_PbD[nq][nv]; 
   }
  }
 in.close();
 ...
 /////////////////////// Calculate systematic error /////////////
  ...
  for(int nq=0; nq<nqbins;nq++){	 
 for(int nv=0;  nv<nvbins; nv++){
   systRC_CD[nq][nv] = TMath::Sqrt(TMath::Power(ratioRC_CD[nq][nv]-ratioRC97_CD[nq][nv],2));
   ...
   systRC_FeD[nq][nv] = TMath::Sqrt(TMath::Power(ratioRC_FeD[nq][nv]-ratioRC97_FeD[nq][nv],2));
   ...
   systRC_PbD[nq][nv] = TMath::Sqrt(TMath::Power(ratioRC_PbD[nq][nv]-ratioRC97_PbD[nq][nv],2));
...
/* 
 ...
 TH2F *hr = new TH2F("hr","hr",default_binning, 2.2, 4.2, default_binning, 0., 0.3);
 ...
 for(int nq=0; nq<nqbins;nq++){
  g_ratioRC_CD[nq] = new TGraphErrors(nvbins, center_v,  syst_percRC_CD[nq],0, 0);
  g_ratioRC_FeD[nq] = new TGraphErrors(nvbins, center_v, syst_percRC_FeD[nq],0,0);
  g_ratioRC_PbD[nq] = new TGraphErrors(nvbins, center_v, syst_percRC_PbD[nq],0, 0);
  ...
  c->Print("~/secure/ExternalsRC/systemRC97_ratio.png");
}
